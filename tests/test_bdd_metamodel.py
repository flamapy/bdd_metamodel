        ...
        (
            ...
            defaultdict(
                float,
                {
                    ...
                    "Server": 0.0518,
                    "Application": 0.9482,
                    "MicroserviceApplication": 0.0347,
                    "UaaServer": 0.0171,
                    "MicroserviceGateway": 0.2730,
                    "Monolithic": 0.6752,
                    "Authentication": 1.0,
                    "HTTPSession": 0.2706,
                    "OAuth2": 0.1341,
                    ...
                    "Cassandra": 0.0107,
                    "MongoDB": 0.0180,
                    "Hibernate2ndLvlCache": 0.6472,
                    "HazelCast": 0.3236,
                    "EhCache": 0.3236,
                    "Development": 0.9707,
                    "H2": 0.6472,
                    "PostgreSQLDev": 0.1079,
                    "MariaDBDev": 0.1079,
                    "MySql": 0.1079,
                    "DiskBased": 0.3236,
                    "InMemory": 0.3236,
                    ...
                    "MariaDB": 0.3236,
                    "PostgreSQL": 0.3236,
                    "ElasticSearch": 0.4854,
                    "SpringWebSockets": 0.4741,
                    "Libsass": 0.4741,
                    "ClusteredSession": 0.4741,
                    "BackEnd": 1.0,
                    "Gradle": 0.5,
                    "Maven": 0.5,
                    "InternationalizationSupport": 0.5,
                    "Docker": 0.5,
                    "TestingFrameworks": 1.0,
                    "Protractor": 0.9482,
                    "Gatling": 1.0,
                    "Cucumber": 1.0,
                },
        ...
        (
            ...
            defaultdict(
                float,
                {
                    ...
                    "Count": 1.0,
                    "Axles2": 0.1111,
                    "MultipleRearAxles": 0.8889,
                    "Axles3": 0.2222,
                    "Axles4": 0.6667,
                    "AdditionalSteeringAxle": 0.3333,
                    "Drivetrain": 1.0,
                    "DrivenAxle1": 0.4444,
                    "DrivenAxles2": 0.3333,
                    "DrivenAxles3": 0.2222,
...
def test_probabilities(path: str, expected: dict):
    ...
    probabilities = {feature: round(prob, PRECISION) for feature, prob in probabilities.items()}
    expected = {feature: round(prob, PRECISION) for feature, prob in expected.items()}
...
@pytest.mark.parametrize(
    ...
    [
        (
            "resources/models/uvl_models/MobilePhone.uvl",
            ["Basic", "Color", "High Resolution", "GPS", "Media", "Camera", "MP3"],
        ),
        (
            "resources/models/uvl_models/JHipster.uvl",
            [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "resources/models/uvl_models/JHipster.uvl",
            ["Gradle", "Maven", "InternationalizationSupport", "Docker"],
        ),
...
@pytest.mark.parametrize(
    "path, expected_total, expected_partial",
    [
...
def test_variability(path: str, expected_total: float, expected_partial: float):
    ...
    assert round(total_variability, PRECISION) == round(expected_total, PRECISION)
    assert round(partial_variability, PRECISION) == round(expected_partial, PRECISION)
...
def test_homogeneity(path: str, expected: float):
    ...
    assert round(homogeneity, PRECISION) == round(expected, PRECISION)


@pytest.mark.parametrize(
    "path, sample_size, expected",
    [
